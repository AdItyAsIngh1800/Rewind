...
| | |
|---|---|
| **E3.1 Detection baseline** | **Goal:** a scored detector on your own benchmark. **Tasks:** pretrained compact YOLO on the 4 tune cases; tune confidence/NMS; measure precision/recall/mAP/FPS/peak-VRAM. **Log every run as an experiment record (§O3) from the very first one** — the registry is worthless if you start it in Week 10. **Result:** first real numbers in the benchmark report. **Prereq:** sampled frames (E2.1) *or* just decode video directly — this phase does not require E2.2. **Links to:** E3.2. |
