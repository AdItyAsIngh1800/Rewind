...
**Accepted limitations on the golden pair** (EXP-0010, EXP-0011, EXP-0013; measured,
not excused): a person at a fifth of their silhouette behind a vehicle is not detected,
and the interval is reported as *Cannot determine* instead (F2, confirmed a visibility
limit rather than a training gap by EXP-0013); two forklifts in turn can share one
per-camera track, which the ID-switch metric cannot express (F8); a pallet being pushed
can flip between two tracks, four switches against the floor of two (F9). Every one is
visible in the report as a gap or a refused link, never as a claim.

**Closed since:** a forklift of a colour absent from the tune data was not detected
(F1); `case_07` and `yolo11n-rewind-v3` fix it without losing the frame-cut forklift,
and the fragment links of F4 disappear with it (EXP-0013).
