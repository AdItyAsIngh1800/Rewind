...
**seven calendar days** (2026-09-09 to 2026-09-15, 260-odd commits). Every phase, gate and
...
| 2 | Detection and tracking meet the benchmark threshold | **Partially met** | Tune cases: every Gate 1 floor met (`docs/gates/gate-1.md`). Held out: robot 0.991/0.998 and ID switches ≤ 2 pass; person recall 0.065 fails (F2, accepted occlusion limit), forklift F02 never detected (F1). Post-golden, `yolo11n-rewind-v3` closes F1 (0.930 recall) and clears the false-link floor, at the cost of one switch-floor breach on `case_06` (F9); F2 is unchanged and confirmed a visibility limit (EXP-0013). `artifacts/benchmark-reports/final.md`. |
...
| 6 | Cross-camera evidence is assembled | **Met on the promoted checkpoint** | Identity links with explicit refusals, scored on false-link rate first. Tune: 0 false links in 62 decisions. Held out with v1: 0.25 against a floor of 0.05 (F4). With v3, whose forklift boxes leave no fragment to link: **0.00** (EXP-0013). Cross-camera recall stays low, which is the layer refusing rather than guessing. |
...
| 10 | Investigator UI supports replay and timeline inspection | **Met** | Three panes on one clock with per-camera offsets; every selection seeks every camera; `make ui-check` 8/8 in Chrome. |
...
| 12 | Critical logic has automated tests | **Met** | 330 tests: unit (geometry, timestamps, evidence states, auth), integration (ingest → observation → event → incident, failures: worker restart, corrupt clip, Redis down, missing camera), API (contract, idempotency, authorization), e2e (golden case → expected timeline), browser (replay sync, evidence navigation). Load tests not written — see §3. |
...
Fourteen met, one partially met, none unmet. Line 2 is the one remaining partial after EXP-0013
closed F1 and v3 cleared line 6's floor; it stands on F2, a visibility limit, and on
F9, a tracker regression that seeing the second forklift uncovered. Both are diagnosed
to a frame: `docs/failures/README.md`.

## 2. Deviation ledger — the debt

64 rows: 20 `IMPROVEMENT`, 26 `NEUTRAL`, 4 `BUG-FIX`, 14 `DEBT` of which 9 are repaid
(three marked repaid in place, six repaid by a later row).
Open debt, and what each costs the reader of the final report:

| Row | Debt | State |
|---|---|---|
| EXP-0011 F1 | The detector could not see a forklift colour absent from the tune set | **Repaid 2026-09-15**: `case_07`, dataset `v1.1`, `yolo11n-rewind-v3` promoted (EXP-0013). The postmortem's verification date is met. |
...
| §L | ML regression: golden benchmark per release | `make bench` and EXP-0010/0011 by hand; not a CI job | Needs the rendered data in CI (42 MB private asset); one afternoon. |
...
- **Mentor walkthrough** of C01 with `docs/gates/gate-4-5-walkthrough.md`. Any change
  it prompts is post-golden, as EXP-0013's already is.
...
system failed the charter's perception floors on unseen data. One of those failures was
closed afterwards, in the open, by the route its own postmortem named (EXP-0013), which
is the strongest evidence in the record that the failure analysis was real work and not
a narrative. That is the sentence a
