...
| **F1** | A forklift of a colour absent from the tune data is never detected (0 of 697, visibility to 0.93) | [F1-case_06-CAM_C-f130.jpg](F1-case_06-CAM_C-f130.jpg): navy F02 (vis 0.95) has no red box; red F01 beside it is found at 0.94 | The fine-tuned detector learned *forklift* as F01's colour; no tune case has a second forklift colour | E9.1, EXP-0010 | **fixed post-golden** (EXP-0013): `case_07`, a tune case with the forklift in F02's navy and parked cut by CAM_B's edge, and a retrain at default augmentation (`yolo11n-rewind-v3`, promoted): F02 found at 0.930 recall and ranked as a cause; `case_02`'s frame-cut forklift kept at 0.996. Colour-blind v2 had traded one for the other |
...
| **F8** | Two forklifts share one track on one camera: F01 leaves CAM_A's view at 12.2 s and F02 enters nearby at 12.6 s, inside ByteTrack's 3 s buffer, and the tracker continues F01's id onto F02 | [F8-case_06-CAM_A-f128.jpg](F8-case_06-CAM_A-f128.jpg): F02 wearing track T001, F01's | A lost track's prediction is re-matched to a different same-class entity appearing where the first went; the ID-switch metric counts an entity changing track, not a track changing entity, so the harness cannot see this | EXP-0013 (present under v2 too, unmeasured) | open: a harness measure for a track that carries two entities, then a shorter buffer or a class-and-appearance gate on re-match, tuned on `case_07` |
| **F9** | A pallet being pushed splits into two tracks and flips between them four times in 1.3 s: four switches against a floor of two | [F9-case_06-CAM_A-f212.jpg](F9-case_06-CAM_A-f212.jpg): PL3 at visibility 0.44 under F02's front gets two pallet boxes | The sliver of a pushed pallet is detected twice; each box seeds a track and the greedy match alternates | EXP-0013 | open: v1 never showed it because it never detected the pusher; a tracker setting (buffer, match) to try on `case_07`, measured on the golden pair beside EXP-0013 |
