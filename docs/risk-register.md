...
| R21 | **Network dependency on Supabase** | Retired (`ADR-0007`: the stack and CI run a local Postgres; Supabase is the optional hosted path) | — | Development blocked when offline or when the project is unreachable | Hosted database accepted deliberately (`ADR-0003`); CI runs against an ephemeral Postgres so tests never depend on it | Development blocked more than once by connectivity |
| R22 | **Free-tier pause and project quota** | Low (`ADR-0007`: nothing on the critical path needs the hosted project) | Low | Project idle approaching seven days; organisation limited to two active projects | Weekly keep-alive GitHub Action; un-pause from the dashboard takes about a minute | Pause interrupts work mid-phase more than once — then upgrade or move local |
| R23 | **Gate 6 weakened by hosted credentials** | Resolved (`ADR-0007`; Gate 6 passed from a fresh clone with no account) | — | "Clone and run" now needs a Supabase project | README documents the four required values and where to find them | A new developer cannot reach a working timeline from the README alone |
